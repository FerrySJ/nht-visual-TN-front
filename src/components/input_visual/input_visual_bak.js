...
const Input_visual = () => {
  ...
  //       // updatedRows[index] = updatedRows[index] || {}; // ตรวจสอบว่า index นั้นมีข้อมูล
  //       updatedRows[index][column] = value;

  //       // ลบแถวเมื่อช่อง input ทั้งหมด (ยกเว้น prod_date, shift) มีค่าเป็น ""
  //       const rowIsEmpty = Object.keys(updatedRows[index])
  //         .filter((key) => key !== "prod_date" && key !== "shift")
  //         .every((key) => updatedRows[index][key] === "");
  ...
  //       // ถ้าแถวสุดท้ายไม่ว่าง ให้เพิ่มแถวใหม่
  //       if (
  //         updatedRows[index][column] !== "" &&
  ...
  const handleInputChange = (e, index, column, cleanedValue = null) => {
    try {
      ...
      console.log("Set rows error :: ", error);
    }
  };
  ...
  const searchMC = async (mc, index) => {
    ...
    try {
      ...
      if (getMc.data.result.data.length > 0) {
        // อัปเดตค่าของ `part` ในแถวที่ระบุ
        setRows((prevRows) => {
          const updatedRows = [...prevRows];
          searchProd(
            updatedRows[index].prod_date,
            mc,
            getMc.data.result.data[0].partNo
          );
          updatedRows[index].part = getMc.data.result.data[0].partNo; // ตั้งค่า part ที่ได้จาก API
          return updatedRows;
        });
        // setLoading("off"); // ไม่ทำตรงนี้เพราะจะรอ search prod
      } else {
        Swal.fire({
          icon: "warning",
          title: "ไม่พบ M/C นี้",
  ...
  const searchProd = async (prod_date, mc, part) => {
    try {
      ...
      if (getProd.data.result.length > 0) {
        let mapData = getProd.data.result
  ...
  const parseBarcode = (barcode) => {
    return {
      mc: barcode.substring(0, 5),
      shift: barcode.substring(5, 6),
      box: barcode.substring(6, 7),
      pos: barcode.substring(7, 8),
    };
  };
        ...
        <div className="card-body">
          ...
          >
            <div className="col-lg-12">
              {loadingScreen()}
              <div className="table-responsive p-0" style={{ height: "calc(100vh - 200px)" }}>
